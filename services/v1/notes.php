...
class Notes extends Module
{
	...
	{
		$query = "SELECT gt.tag, gt.tag_id, gt.player_created, gt.media_id FROM note_tags LEFT JOIN ((SELECT tag, tag_id, player_created FROM game_tags WHERE game_id = '{$gameId}') as gt) ON note_tags.tag_id = gt.tag_id WHERE note_id = '{$noteId}'";
		$result = Module::query($query);
		$tags = array();
		while($tag = mysql_fetch_object($result))	
			$tags[] = $tag;

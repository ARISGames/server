...
abstract class Module
{
  ...
  protected function playerHasNote($intGameID, $intPlayerID, $qty)
  {
    ...
    $query = "SELECT note_id FROM notes WHERE owner_id = '{$intPlayerID}' AND parent_note_id = 0 AND complete = '1'";
    NetDebug::trace($query);
    $result = @mysql_query($query);
  ...
  protected function playerHasNoteWithComments($intGameID, $intPlayerID, $qty)
  {
    ...
    $query = "SELECT note_id FROM notes WHERE game_id = '{$intGameID}' AND owner_id = '{$intPlayerID}' AND complete = '1'";
    NetDebug::trace($query);
    $result = @mysql_query($query);

...
class ar_targets extends dbconnection
{
  ...
  public static function uploadARTargetDB($pack)
  {
    ...
    if(!is_dir($fsfolder."/thumbs")) mkdir($fsfolder."/thumbs",0777,true); //will create $fsfolder and $fsfolder/thumbs
    ...
    for($i = 0; $i < $zip->numFiles; $i++)
    {
      ...
      if($name == $filenamebase.".xml") $xmlpresent = true;
      else if($name == $filenamebase.".dat") $datpresent = true;
    ...
    //situate contents
    rename($fsfolder."/".$filenamebase.".xml", $fsfolder."/vuforiadb.xml");
    rename($fsfolder."/".$filenamebase.".dat", $fsfolder."/vuforiadb.dat");
    //cleanup
    unlink($fspath); //zip
    ...
    $cur_targets = ar_targets::getARTargetsForGame($pack)->data;
    ...
    for($j = 0; $j < count($names); $j++)
      rename($fsfolder."/thumbs/".$names[$j].".tex.jpg", $fsfolder."/thumbs/".$names[$j].".jpg");

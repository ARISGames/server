...
class GeoLocation {
   ...
   * spherical approximation of the figure of the Earth is approximately
   * 6371.01 kilometers.
   * @return Boundary object with two GeoLocation objects (min,max) such that:<ul>
   ...
   */
	public function boundingCoordinates($distance, $unit_of_measurement) {
        ...
        $coords = new \stdClass;
        $coords->wat = 5;
        $coords->min = GeoLocation::fromRadians($minLat, $minLon);
        $coords->max = GeoLocation::fromRadians($maxLat, $maxLon);
